...
from models_diff import MSDP_diff
import random
def same_seeds(seed):
...
def main():
    ...
    channels=8
    save_dir = 'msdp-diff-nh({}{}{}{}{})_hd_{}_c_{}_ncc_{}_reg_{}_lr_{}_4r/'.format(*num_heads, head_dim,channels,*weights, lr)
    ...
    Initialize model
    '''
    model = MSDP_diff(img_size, num_heads=num_heads, channels=channels, head_dim=head_dim, scale=1)
    model.cuda()
    ...
    train_set = datasets.TrainDatasetS2S(natsorted(glob.glob(train_dir + '*.pkl')), transforms=train_composed)
